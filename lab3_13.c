...
void generate(float **M1_p, float **M2_p, const uint32_t len, const uint32_t A,
    ...
    *M2_p = (float *)malloc(sizeof(float) * (len / 2));
    ...
    #if defined(CYCLE_INDEX) && CYCLE_INDEX == 1
    #pragma omp parallel for default(none) shared(M1_p, M2_p, len, fixed, A, seed) schedule(SCHEDULE_TYPE, CHUNK_SIZE)
    #else
...
void map(float * const M1, float * const M2, const uint32_t len) {
    #if defined(CYCLE_INDEX) && CYCLE_INDEX == 2
    #pragma omp parallel for default(none) shared(M1, M2, len) schedule(SCHEDULE_TYPE, CHUNK_SIZE)
    #else
    #pragma omp parallel for default(none) shared(M1, M2, len) schedule(guided)
    #endif
    for (uint32_t i = 0; i < len; i++) {
        M1[i] = sinh(M1[i]);
        M1[i] = M1[i] * M1[i];
    ...
    #if defined(CYCLE_INDEX) && CYCLE_INDEX == 3
    #pragma omp parallel for default(none) shared(M1, M2, M2_copy, len) schedule(SCHEDULE_TYPE, CHUNK_SIZE)
    #else
    ...
    #if defined(CYCLE_INDEX) && CYCLE_INDEX == 4
    #pragma omp parallel for default(none) shared(M2, M2_copy, len) schedule(SCHEDULE_TYPE, CHUNK_SIZE)
    #else
...
void merge(float * const M1, float * const M2, const uint32_t len) {
    #if defined(CYCLE_INDEX) && CYCLE_INDEX == 5
    #pragma omp parallel for default(none) shared(len, M1, M2) schedule(SCHEDULE_TYPE, CHUNK_SIZE)
    #else
...
float reduce(float * const M2, const uint32_t len, const int no_sort) {
    ...
    #if defined(CYCLE_INDEX) && CYCLE_INDEX == 6
    #pragma omp parallel for default(none) private(tmp) shared(len, M2, compare, sum) schedule(SCHEDULE_TYPE, CHUNK_SIZE) reduction(+:sum)
    #else
    #pragma omp parallel for default(none) private(tmp) shared(len, M2, compare, sum) schedule(guided) reduction(+:sum)
    #endif
    for (uint32_t i = 0; i < len / 2; i++) {
...
int main(int argc, char *argv[]) {
    ...
    int32_t N;
    float iteration_result = 0; struct timeval T1, T2;
    int64_t delta_ms;
