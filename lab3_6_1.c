...
void generate(float **M1_p, float **M2_p, const uint32_t len, const uint32_t A,
    ...
    *M2_p = (float *)malloc(sizeof(float) * (len / 2));
    ...
    #pragma omp parallel for default(none) shared(M1_p, M2_p, len, fixed, A, seed) schedule(static, CHUNK_SIZE)
    #else
...
void map(float * const M1, float * const M2, const uint32_t len) {
    ...
    #pragma omp parallel for default(none) shared(M1, M2, len) schedule(guided, CHUNK_SIZE)
    #else
    ...
    #pragma omp parallel for default(none) shared(M1, M2, M2_copy, len) schedule(static, CHUNK_SIZE)
    #else
    ...
    #pragma omp parallel for default(none) shared(M2, M2_copy, len) schedule(dynamic, CHUNK_SIZE)
    #else
...
void merge(float * const M1, float * const M2, const uint32_t len) {
    ...
    #pragma omp parallel for default(none) shared(len, M1, M2) schedule(guided, CHUNK_SIZE)
    #else
...
float reduce(float * const M2, const uint32_t len, const int no_sort) {
    ...
    #pragma omp parallel for default(none) private(tmp) shared(len, M2, compare, sum) schedule(guided, CHUNK_SIZE) reduction(+:sum)
    #else
